...
function start(fn) {
  ...
  db.on_ready(function(err){
      ...
      fnc.clear_private_data_tables(function(err){
            if (err){
                update_status('fnc.clear_private_data_tables: error');
                return fn(err_info(err,'fnc.clear_private_data_tables error'));
            }
            update_status('fnc.clear_private_data_tables: success');
